...
void RPiComm_sendRawUWBDataWithTAG(float distances[3], bool anchor_status[3], 
                           ...
                           bool tag_data_valid) {
    // NUEVO FORMATO EXTENDIDO: Lista [d1,d2,d3,bat_carro,curr_L,curr_R,bat_tag,modo_tag,joystick_tag,q_i,q_j,q_k,q_r,q_acc,bno_stab,steps]
    ...
    for (int i = 0; i < 3; i++) {
        if (anchor_status[i] && !isnan(distances[i])) {
            ...
            RPiSerial.print(-10);  // Valor inválido
        }
        RPiSerial.print(",");
    }
    ...
    // Estado BNO y pasos
    if (tag_data_valid) {
        RPiSerial.print(tag_bno_stability);
        RPiSerial.print(",");
        RPiSerial.print(tag_step_count);
    } else {
        RPiSerial.print(-1);
        RPiSerial.print(",");
        RPiSerial.print(-1);
